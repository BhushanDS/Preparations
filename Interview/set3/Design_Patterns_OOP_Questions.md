...
## Q11: Implement the Decorator pattern in modern C++. How does it differ from inheritance and mixins?

### Answer:

**Classic Decorator (Wrapping):**
```cpp
// Interface
class Stream {
public:
    ...
    virtual std::vector<std::byte> read(size_t n) = 0;
};
...
class FileStream : public Stream {
    std::fstream file_;
public:
    explicit FileStream(const std::string& path) : file_(path, std::ios::binary | std::ios::in | std::ios::out) {}
    void write(std::span<const std::byte> data) override { file_.write(reinterpret_cast<const char*>(data.data()), data.size()); }
    std::vector<std::byte> read(size_t n) override { /* ... */ return {}; }
};

// Decorator base ? forwards to wrapped stream
class StreamDecorator : public Stream {
protected:
    std::unique_ptr<Stream> wrapped_;
public:
    explicit StreamDecorator(std::unique_ptr<Stream> s) : wrapped_(std::move(s)) {}
    void write(std::span<const std::byte> data) override { wrapped_->write(data); }
    std::vector<std::byte> read(size_t n) override { return wrapped_->read(n); }
};
...
class EncryptedStream : public StreamDecorator {
    CryptoKey key_;
public:
    EncryptedStream(std::unique_ptr<Stream> s, CryptoKey key) 
        : StreamDecorator(std::move(s)), key_(std::move(key)) {}
    void write(std::span<const std::byte> data) override {
        auto encrypted = encrypt(data, key_);
...
public:
    ...
    void flush() { wrapped_->write(buffer_); buffer_.clear(); }
};
...
// Compose at runtime ? ANY combination of decorators:
...
// Write path: compress ? encrypt ? buffer ? file
```

**Modern C++ Alternative ? Compile-time Decorator with CRTP:**
```cpp
// Zero-overhead decorator using templates (no vtable, no heap allocation)
template<typename Base>
class WithLogging : public Base {
public:
    using Base::Base;
    void write(std::span<const std::byte> data) {
        spdlog::debug("Writing {} bytes", data.size());
        Base::write(data);
    }
};

template<typename Base>
class WithMetrics : public Base {
public:
    using Base::Base;
    void write(std::span<const std::byte> data) {
        auto start = std::chrono::steady_clock::now();
        Base::write(data);
        metrics_.record(std::chrono::steady_clock::now() - start);
    }
};

// Stack decorators at compile time:
using MonitoredFileStream = WithLogging<WithMetrics<FileStream>>;
// Zero vtable overhead, fully inlinable!
```

**Decorator vs Inheritance vs Mixins:**
| Aspect | Decorator | Inheritance | Mixins (CRTP) |
|--------|-----------|-------------|---------------|
| Composition | Runtime | Compile-time | Compile-time |
| Overhead | vtable + heap | None | None |
| Combinability | Any N of M | Fixed hierarchy | Any N of M |
| Adding behavior | Open/closed | Modify base | Add template layer |
| Type identity | Same interface | IS-A | Static type changes |

### Explanation:
Decorator is the go-to pattern when you need to layer behaviors (logging, caching, compression, encryption) without creating an explosion of subclasses. At senior level, know BOTH the classic runtime version (for plugin systems) and the compile-time CRTP version (for zero-overhead). This is asked at infrastructure companies (Cloudflare, AWS) where I/O pipeline composition is common.

---

## Q12: Explain Policy-Based Design. How does it compare to Strategy pattern?

### Answer:

**Policy-Based Design (Alexandrescu/Modern C++ Design):**
```cpp
// Policies are compile-time "strategy" selections
// Zero-overhead: policies are fully inlined, no vtable
...
struct MultiThreaded {
    struct Lock {
        std::mutex& mtx_;
        Lock(std::mutex& m) : mtx_(m) { mtx_.lock(); }
        ~Lock() { mtx_.unlock(); }
    };
};
...
struct PoolAllocation {
    template<typename T>
    static T* allocate() { return pool<T>().get(); }
    template<typename T>
    static void deallocate(T* p) { pool<T>().release(p); }
};
...
// The class is parameterized by policies ? user selects at compile time
template<
    typename ThreadingPolicy = SingleThreaded,
    typename AllocationPolicy = HeapAllocation,
...
public:
    void release() {
        LoggingPolicy::log("Releasing smart pointer");
...
using SafePtr = SmartPointer<MultiThreaded, PoolAllocation, ConsoleLogging>;
using FastPtr = SmartPointer<SingleThreaded, HeapAllocation, NoLogging>;
// Each combination is a distinct TYPE ? zero runtime overhead
```

**Real-World Example ? Configurable Container:**
```cpp
template<
    typename Key,
    typename Value,
    typename HashPolicy = std::hash<Key>,
    typename ComparePolicy = std::equal_to<Key>,
    typename GrowthPolicy = ExponentialGrowth,
    typename ProbePolicy = LinearProbing
>
class HashMap {
    // HashPolicy determines hash function
    // ComparePolicy determines key equality
    // GrowthPolicy determines when/how to resize
    // ProbePolicy determines collision resolution
};

// std::unordered_map IS policy-based design:
// std::unordered_map<K, V, Hash, KeyEqual, Allocator>
```

**Strategy (Runtime) vs Policy (Compile-time):**
| Aspect | Strategy Pattern | Policy-Based Design |
|--------|-----------------|-------------------|
| Binding time | Runtime | Compile-time |
| Overhead | vtable + indirection | Zero (inlined) |
| Flexibility | Change at runtime | Fixed at compile time |
| Combinatorics | Manual composition | Automatic via templates |
| Binary size | Smaller (shared code) | Larger (each combo is unique) |
| Use when | Behavior changes at runtime | Performance-critical, behavior fixed at build |

### Explanation:
Policy-based design is THE advanced C++ pattern that separates senior from principal engineers. It's the foundation of `std::allocator`, smart pointer implementations, and high-performance libraries (Folly, Abseil, Boost). Interviewers at companies with performance-critical C++ (game engines, HFT, databases) specifically ask about this pattern because it demonstrates deep template expertise and understanding of zero-overhead abstractions.

---

# ?? INTERVIEWER GUIDE ? Set 3 Scoring & Evaluation

---

## Pattern Knowledge Depth Check

| Level | Expectation |
|-------|-------------|
| **Junior** | Knows Singleton, Factory, Observer names. Can implement simple Factory. |
| **Mid** | Implements most GoF patterns. Knows when to use Strategy vs State. |
| **Senior** | Discusses trade-offs (virtual vs CRTP), knows anti-patterns, designs with DI. |
| **Staff** | Designs pattern combinations for real architectures. Knows when NOT to use patterns. |

## ?? Green Flags

```
? Says "I'd use Strategy here because behavior changes at runtime, but Policy if it's fixed"
? Mentions SOLID violations when critiquing code
? Knows the "Self-Registering Factory" ? shows they've built extensible systems
? Discusses virtual dispatch overhead vs compile-time alternatives (CRTP, concepts)
? Can explain Type Erasure and where std::function uses it internally
? Mentions Dependency Injection for testability
? Knows when Observer pattern needs weak references to prevent leaks
? Can design a Command system with Macro/Composite commands
```

## ?? Red Flags

```
? Only knows Singleton (and uses it everywhere)
? Can't explain the difference between Strategy and Template Method
? Thinks patterns are "good" without understanding costs (virtual call overhead, complexity)
? Never mentions testing or DI ? patterns exist in isolation
? Uses inheritance hierarchies deeper than 2-3 levels
? Creates God classes or Managers that do everything
? Doesn't know CRTP or Type Erasure ? stops at GoF basics
```

## Architecture Patterns (Beyond GoF)

| Pattern | When | C++ Application |
|---------|------|-----------------|
| **CQRS** | Read/write have different scaling needs | Trading (fast reads, queued writes) |
| **Event Sourcing** | Need complete audit trail / undo | CAD (parametric history), Finance (trade log) |
| **Hexagonal (Ports & Adapters)** | Core logic must be testable without I/O | Any production system |
| **ECS (Entity-Component-System)** | Game objects with varying capabilities | Gaming, simulation |
| **Actor Model** | Message-passing concurrency | Distributed systems, Erlang-style C++ |
