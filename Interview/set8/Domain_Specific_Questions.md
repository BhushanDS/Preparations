...
## Q11: Explain client-side prediction, server reconciliation, and rollback netcode for multiplayer games.
...
Game tick rate: 60 Hz (16.6ms per frame)
Without prediction: player presses "move right" ? 100ms later character moves ? unplayable

Solution: Client predicts locally, server authoritative, reconcile differences
```

**Three Pillars of Game Netcode:**
```
1. CLIENT-SIDE PREDICTION
   Client applies input IMMEDIATELY (don't wait for server)
   Player sees instant response
   
2. SERVER RECONCILIATION  
   Server processes inputs, sends authoritative state
   Client compares prediction with server result
   If mismatch: correct by replaying inputs from server's last confirmed state
   
3. ENTITY INTERPOLATION (other players)
   You can't predict OTHER players' inputs
   Instead: buffer their states and interpolate between known positions
   Display them slightly in the past (100ms behind)
```
...
struct Input {
    uint32_t tick;
    float moveX, moveZ;   // Movement input
    float lookDeltaX, lookDeltaY;
    bool jump, shoot;
};

class ClientPrediction {
    // Ring buffer of predicted states ? one per tick
    static constexpr int BUFFER_SIZE = 128;  // ~2 seconds at 60Hz
    PlayerState predictions_[BUFFER_SIZE];
    Input inputHistory_[BUFFER_SIZE];
    uint32_t lastConfirmedTick_ = 0;
    
public:
    // Called every client tick (16.6ms)
    PlayerState predictTick(const Input& input, const PlayerState& current) {
        // Save input for potential replay
        inputHistory_[input.tick % BUFFER_SIZE] = input;
        
        // Simulate locally (same physics code as server!)
        PlayerState predicted = simulateStep(current, input);
        predictions_[input.tick % BUFFER_SIZE] = predicted;
        
        // Send input to server (don't send state ? server computes it)
        network_.send(input);
        
        return predicted;  // Display this immediately
    }
    
    // Called when server state arrives (authoritative)
    PlayerState reconcile(const PlayerState& serverState) {
        lastConfirmedTick_ = serverState.tick;
        
        // Compare server state with our prediction at that tick
        auto& predicted = predictions_[serverState.tick % BUFFER_SIZE];
        
        if (statesMatch(predicted, serverState)) {
            return getCurrentPrediction();  // Prediction was correct!
        }
        
        // MISMATCH: Server is authoritative ? replay from server state
        PlayerState corrected = serverState;
        for (uint32_t t = serverState.tick + 1; t <= currentTick_; t++) {
            corrected = simulateStep(corrected, inputHistory_[t % BUFFER_SIZE]);
            predictions_[t % BUFFER_SIZE] = corrected;  // Update predictions
        }
        
        return corrected;  // Smoothly blend to corrected position
    }
    
private:
    // MUST be deterministic ? same input ? same output on client AND server
    PlayerState simulateStep(const PlayerState& state, const Input& input) {
        PlayerState next = state;
        next.tick = input.tick;
        Vec3 moveDir = {input.moveX, 0, input.moveZ};
        next.velocity = moveDir * MOVE_SPEED;
        if (input.jump && onGround(state)) next.velocity.y = JUMP_SPEED;
        next.velocity.y -= GRAVITY * TICK_DELTA;
        next.position += next.velocity * TICK_DELTA;
        return next;
    }
};
...
class EntityInterpolation {
    ...
    static constexpr auto INTERP_DELAY = std::chrono::milliseconds(100);
    
public:
    void addSnapshot(const PlayerState& state) {
        snapshots_.push_back({state, std::chrono::steady_clock::now()});
        while (snapshots_.size() > 20) snapshots_.pop_front();
    }
    
    PlayerState getInterpolatedState() {
        // Render this entity 100ms in the past
        auto renderTime = std::chrono::steady_clock::now() - INTERP_DELAY;
        
        // Find the two snapshots that bracket renderTime
        for (size_t i = 1; i < snapshots_.size(); i++) {
            if (snapshots_[i].receivedAt >= renderTime) {
                float t = computeInterpolationFactor(
                    snapshots_[i-1].receivedAt, snapshots_[i].receivedAt, renderTime);
                return lerp(snapshots_[i-1].state, snapshots_[i].state, t);
            }
        }
        return snapshots_.back().state;  // Extrapolate if no future snapshot
    }
};
```

**Lag Compensation (Server-side for hit detection):**
```
When player A shoots at player B:
  Player A sees B at their position 100ms ago (interpolation delay + latency)
  Server must "rewind" B to where A SAW them ? check hit ? apply damage

This is why you sometimes get "shot around corners" in FPS games:
  On the shooter's screen, you were visible. Server respects that.
```

### Explanation:
Game networking is a domain-specific skill asked at game studios (Riot, Epic, Blizzard) and increasingly at companies building collaborative real-time apps (Figma, Miro). The three pillars ? prediction, reconciliation, interpolation ? are also applicable to collaborative editing (OT/CRDT), distributed simulation, and any system where latency-hiding matters.

---

# ?? INTERVIEWER GUIDE ? Set 8 Scoring & Evaluation

---

## Domain Expertise Interview Strategy

> **Key Insight for Candidates**: You don't need to be an expert in ALL domains. Pick 1-2 and go DEEP. Interviewers respect depth over breadth.

| Domain | Companies That Ask | Key Topics to Master |
|--------|-------------------|---------------------|
| **CAD/3D** | Autodesk, PTC, Dassault, Siemens, Onshape | B-Rep, NURBS, Boolean ops, topological naming, large assembly |
| **Gaming** | Epic, Riot, Blizzard, EA, Naughty Dog | ECS, game loop, physics, networking, rendering pipeline |
| **Finance/HFT** | Citadel, Jane Street, Two Sigma, Goldman, DE Shaw | Lock-free, kernel bypass, FPGA, market data, risk engines |
| **Enterprise/IBM** | Rocket Software, IBM, SAP, Oracle | HA/DR, journaling, mainframe modernization, COBOL interop |
| **Embedded/IoT** | Qualcomm, NVIDIA, ARM, Apple | RTOS, DMA, interrupt handling, power management |

## How to Present Domain Knowledge

```
FRAMEWORK: "In my domain, the unique challenge is X, and I solved it using Y"

CAD Example:
  "The topological naming problem means that when you edit feature 3,
   features 4-20 might break because they reference faces by index.
   I implemented persistent naming using creation history encoding,
   which reduced model corruption from 5% to 0.1%."

Finance Example:
  "At peak, we process 10M market data messages/second with <5?s
   tick-to-trade latency. I eliminated heap allocation on the hot path
   using pre-allocated object pools and achieved deterministic latency."

iCluster Example:
  "Our HA/DR replication system handles journal-based CDC for 1000+
   objects simultaneously. I designed the staging store architecture
   that provides exactly-once delivery semantics with crash recovery."
```

## Cross-Domain Skills That Transfer

| Skill | CAD | Gaming | Finance | Enterprise |
|-------|-----|--------|---------|------------|
