...
## Q11: Explain the three-way comparison operator (`<=>`) and designated initializers. How do they change API design?
...
// Before C++20: 6 comparison operators to implement
// After C++20: ONE defaulted operator generates all 6
...
// Custom ordering: semantic versioning rules
struct SemanticVersion {
    ...
    std::strong_ordering operator<=>(const SemanticVersion& rhs) const {
        ...
        if (auto cmp = patch <=> rhs.patch; cmp != 0) return cmp;
        
        // Pre-release: empty means "release" which is GREATER than any pre-release
    ...
    // Still need == separately if <=> doesn't generate it
    bool operator==(const SemanticVersion&) const = default;
};

// Three ordering categories:
// std::strong_ordering  ? a == b implies substitutability (like int)
// std::weak_ordering    ? a == b means "equivalent" not "identical" (case-insensitive string)
// std::partial_ordering ? some values are incomparable (like NaN in float)

struct CaseInsensitiveString {
    std::string value;
    
    std::weak_ordering operator<=>(const CaseInsensitiveString& rhs) const {
        // "Hello" == "HELLO" in weak ordering
        auto lowerL = toLower(value);
        auto lowerR = toLower(rhs.value);
...
```cpp
// Named fields at construction (like C99, finally in C++)
...
// Clear, self-documenting construction ? order must match declaration order
auto cfg = NetworkConfig{
    .host = "prod-server.corp.com",
    .port = 443,
    .maxConnections = 10000,
    .timeout = std::chrono::seconds{5},
    .useTLS = true,
    // .retryCount uses default (3)
};

// WHY this matters for API design:
// 1. Eliminates "boolean blindness": connect(true, false, true) ? what do those mean?
// 2. Named parameters without builder pattern overhead
// 3. Default values clearly visible in struct definition
// 4. Compiler error if you use wrong order (C++ enforces declaration order)
// 5. New fields can be added with defaults without breaking callers

// Pattern: "Options struct" replaces long parameter lists
class HttpClient {
    struct Options {
...
public:
    explicit HttpClient(Options opts = {}) : opts_(std::move(opts)) {}
    
    // Usage: auto client = HttpClient({.proxy = "http://proxy:8080", .readTimeout = 60s});
};
```

**Concepts Subsumption ? Advanced C++20:**
```cpp
// Subsumption: more constrained overload is preferred
template<typename T>
...
template<typename T>
concept OrderedHashable = Hashable<T> && std::totally_ordered<T>;

// Compiler picks MOST constrained viable overload
template<Hashable T>
void insert(const T& val) { /* hash-based */ }

template<OrderedHashable T>
void insert(const T& val) { /* ordered + hash ? use sorted structure */ }

int x = 42;
insert(x);  // Calls OrderedHashable overload (more constrained, int satisfies both)
```

### Explanation:
These three features fundamentally change how senior engineers design APIs:
- `<=>` eliminates boilerplate while giving precise control over ordering semantics
- Designated initializers enable the "Options struct" pattern (like Go/Rust struct literals)
- Concepts subsumption enables type-safe overload resolution without SFINAE tricks

At 10+ years, interviewers expect you to design types with comparison semantics (not just implement them) and to use designated initializers for configuration-heavy systems.

---

## Q12: Compare C++ to Rust for systems programming. When would you still choose C++ and when would Rust be better?
...
| Aspect | C++ | Rust |
|--------|-----|------|
| **Memory safety** | Manual (RAII, smart ptrs, sanitizers) | Compile-time (borrow checker) |
| **Concurrency safety** | Manual (mutexes, atomics, TSan) | Compile-time (Send/Sync traits) |
| **Undefined behavior** | Possible (dangling, races, overflow) | Impossible in safe code |
| **Compilation speed** | Medium (precompiled headers, modules) | Slow (full analysis per crate) |
| **Ecosystem** | Massive (40+ years, every domain) | Growing fast (5+ years mainstream) |
| **Interop** | C ABI native, FFI straightforward | FFI via `unsafe`, C ABI compatible |
| **Learning curve** | High (UB, templates, corner cases) | High (borrow checker, lifetimes) |
| **Performance** | Peak (same as Rust for equivalent code) | Peak (same as C++ for equivalent code) |
| **Zero-cost abstractions** | Yes (templates, constexpr) | Yes (generics, traits) |
| **Package management** | Fragmented (vcpkg, Conan, CMake) | Unified (cargo) |
| **Build system** | Fragmented (CMake, Bazel, Meson) | Unified (cargo) |
| **IDE support** | Excellent (CLion, VS, VS Code) | Good (rust-analyzer) |

**Choose C++ when:**
```
? Existing C++ codebase (migration cost too high)
? Team expertise is C++ (hiring, training)
? Domain requires mature C++ libraries (Qt, Boost, OpenCASCADE, CUDA)
? Need C++ template metaprogramming (expression templates, Eigen)
? Platform requires it (IBM i ILE, legacy embedded)
? Real-time with deterministic allocation (custom allocators)
? Interop with existing C/C++ ecosystem (game engines, CAD kernels)
```

**Choose Rust when:**
```
? Greenfield systems project (no legacy)
? Security-critical code (memory safety guaranteed)
? Concurrent/parallel code (data races impossible)
? Need fearless refactoring (compiler catches everything)
? DevOps/infrastructure tools (single binary, no runtime deps)
? WebAssembly targets
? Team willing to learn (steep but worth it for safety)
```

**C++ Interop with Rust (real-world pattern):**
```cpp
...
// Rust generates: extern "C" functions with proper ABI
// No manual FFI, no unsafe on the Rust side (cxx validates)
```

### Explanation:
At staff/principal level, language choice is an architectural decision. The honest answer: C++ and Rust are equally fast, but Rust eliminates entire classes of bugs at compile time. Choose C++ for ecosystem/team/legacy reasons. Choose Rust for safety/correctness/new projects. Many teams are writing new components in Rust while maintaining C++ codebases ? this is the pragmatic path.

---

# ?? INTERVIEWER GUIDE ? Set 1 Scoring & Evaluation

> **Perspective**: Senior Architect / Principal Engineer / Senior Recruiter at Google, Microsoft, Bloomberg, Citadel, Jane Street

---

## Scoring Rubric for Modern C++ Questions

| Level | Score | What to Look For |
|-------|-------|------------------|
| **Junior (0-3 yrs)** | 1-3/10 | Knows syntax, can write `unique_ptr`, knows `auto`. Struggles with move semantics nuances. |
| **Mid (3-6 yrs)** | 4-6/10 | Solid on variant/optional/expected. Understands SFINAE conceptually. Can write coroutine generator with guidance. |
| **Senior (6-10 yrs)** | 7-8/10 | Fluent in concepts, ranges, coroutines. Understands memory orderings. Can discuss trade-offs between C++17/20/23 features in production. |
| **Staff/Principal (10+ yrs)** | 9-10/10 | Designs APIs using these features. Knows ABI implications. Can discuss compiler codegen (assembly) for move vs copy. Understands interaction between features (concepts + coroutines + ranges). |
...
```
? Mentions ABI implications when discussing variant/any size
? Discusses compiler error messages when comparing SFINAE vs Concepts
? Knows that std::move doesn't move ? it's just a cast
? Mentions noexcept on move ops and why STL cares (move_if_noexcept)
? Can explain coroutine frame allocation and HALO optimization
? Discusses RVO/NRVO before even asked about moves
? Knows constexpr transient vs non-transient allocation distinction
? Mentions modules' impact on build times with specific numbers
? Can compare weak_ordering vs strong_ordering vs partial_ordering with real examples
? Discusses cache effects when choosing between std::map vs flat_map
? Brings up sanitizers (ASan, TSan, UBSan) when discussing safety
```

## ?? Red Flags (No-Hire Signals)

```
? Confuses std::variant with std::any (doesn't understand type-safety distinction)
? Thinks std::move actually moves data
? Can't explain why shared_ptr is slower than unique_ptr
? Doesn't know Rule of Zero ? always writes Rule of Five
? Uses raw owning pointers "for performance" without measurement
? Can't explain what happens when you copy a unique_ptr (should fail to compile)
? Thinks SFINAE and Concepts are "the same thing"
? Doesn't understand why noexcept matters on move constructors
? Writes auto everywhere without understanding type deduction rules
? Never heard of std::expected or monadic operations
? Can't discuss any C++23/26 feature ? shows lack of continuous learning
```

---

## Follow-Up Deep Dives (What senior interviewers probe next)

**After Q1 (variant/optional/expected):**
- "What's the size of `std::variant<int, std::string>` and why?"
- "Can a variant become valueless? When? How do you guard against it?"
- "Compare `std::expected` to Rust's `Result<T,E>` ? what's missing in C++?"

**After Q2 (Concepts):**
- "Show me a concept that checks if a type has a `serialize()` method returning `std::string`."
- "What's concept subsumption and why does it matter for overload resolution?"
- "Can concepts replace `static_assert`? When would you still prefer `static_assert`?"

**After Q3 (Coroutines):**
- "What is the coroutine frame and where is it allocated?"
- "Can you write a coroutine that does async I/O with Boost.Asio?"
- "What's the difference between `co_await` on a `std::suspend_always` vs `std::suspend_never`?"

**After Q6 (Move Semantics):**
- "Write a class that is movable but NOT copyable. When is this pattern useful?"
- "What's the copy-and-swap idiom? Show the exception-safe assignment operator."
- "Why does `std::vector::push_back` prefer `noexcept` move? Show what happens without it."

**After Q10 (Smart Pointers):**
- "You have a shared_ptr<Widget> and need to pass `this` to a callback. How? (enable_shared_from_this)"
- "What's the aliasing constructor of shared_ptr? When would you use it?"
- "How does make_shared differ from shared_ptr(new T) in terms of memory layout?"

---

## Q13: Explain Template Metaprogramming. Show compile-time computation examples.
...
// Get type at index
template<typename List, size_t N> struct TypeAt;
template<typename T, typename... Ts>
struct TypeAt<TypeList<T, Ts...>, 0> { using type = T; };
template<typename T, typename... Ts, size_t N>
struct TypeAt<TypeList<T, Ts...>, N> { using type = typename TypeAt<TypeList<Ts...>, N-1>::type; };
...
struct Contains<TypeList<Target, Ts...>, Target> : std::true_type {};
template<typename T, typename... Ts, typename Target>
struct Contains<TypeList<T, Ts...>, Target> : Contains<TypeList<Ts...>, Target> {};

// === Practical: Compile-time dispatch table ===
template<typename... Handlers>
class EventDispatcher {
    std::tuple<Handlers...> handlers_;
public:
    EventDispatcher(Handlers... h) : handlers_(std::move(h)...) {}
    
    template<typename Event>
    void dispatch(const Event& e) {
        std::apply([&e](auto&... handlers) {
            (tryHandle(handlers, e), ...);  // Fold expression + SFINAE
        }, handlers_);
    }
    
private:
    template<typename H, typename E>
    static auto tryHandle(H& h, const E& e) -> decltype(h.handle(e), void()) {
        h.handle(e);
    }
    template<typename...> static void tryHandle(...) {} // Fallback: no-op
};
```

### Explanation:
- **TMP is dying** ? `constexpr` functions and concepts replace most metaprogramming
- Still valuable for: type-level computation, trait libraries, expression templates
- **C++26 Reflection** will eliminate even more TMP use cases
- **Interview signal**: Understanding when NOT to use TMP shows maturity

---

## Q14: What is Argument-Dependent Lookup (ADL) and why does it cause subtle bugs?
...
namespace ui {
    void draw(const auto& widget) { /* ... */ }
}

int main() {
    graphics::Point p{1.0, 2.0};
    draw(p);  // ADL finds graphics::draw because Point is in graphics::
    // No need to write graphics::draw(p) ? convenient BUT dangerous
}

// === The danger: unintended overload ===
namespace math {
    struct Vector { double x, y, z; };
    // This swap was meant for internal use only:
    void swap(Vector& a, Vector& b) { /* custom implementation */ }
}

math::Vector v1, v2;
swap(v1, v2);  // ADL calls math::swap instead of std::swap!
// If math::swap has different semantics ? subtle bug
```

**ADL Rules:**
1. When calling an unqualified function, compiler searches namespaces of argument types
2. This is how `std::cout << x` works ? `operator<<` found via ADL
3. **Prevent unwanted ADL**: Use qualified calls `std::swap(a, b)` or `(swap)(a, b)` (parens suppress ADL)

### Explanation:
At 10+ years, you've been bitten by ADL at least once. It's the reason `using namespace std` in headers is dangerous. Senior engineers know to use `std::ranges::sort` (which disables ADL) over unqualified `sort`. This question tests deep language understanding.

---

## Q15: Explain `std::string_view` vs `std::string`. What are the lifetime pitfalls?

### Answer:

```cpp
// string_view = non-owning view of character data (pointer + size)
void processName(std::string_view name) {
    // No copy, no allocation ? just pointer arithmetic
    auto first = name.substr(0, name.find(' '));
}

// === THE DANGER: Dangling reference ===
std::string_view dangerous() {
    std::string temp = "hello";
    return temp;  // DANGLING! temp destroyed, string_view points to freed memory
}

// === Safe patterns ===
// ? Parameter that's used synchronously
void logMessage(std::string_view msg) { spdlog::info(msg); }

// ? Storing in a member ? use std::string
class Config {
    std::string name_;  // NOT string_view ? would dangle
public:
    Config(std::string_view name) : name_(name) {} // Copy into string
};

// ? Return from function only if source outlives caller
...
|---|---|
| Function parameters (read-only) | Class members / data storage |
| Parsing / tokenizing | Returning from factory functions |
| Lookup keys in maps | Anything that outlives the source data |
| Performance-critical hot paths | When ownership transfer needed |

**Interview signal**: Candidates who immediately mention dangling lifetime risk demonstrate production experience.
